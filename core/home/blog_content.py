...
ARTICLES = [
    ...
    _article(
        29, 'sql-techniques-and-the-bottom-line',
        'Your Margins Are on Fire. Your Executives Brought Marshmallows.',
        'Warehouse efficiency',
        'Avoidable SQL waste burns margins and resources. Once leadership knows about it, the recurring bill is a decision. Own the damn invoice.',
        [
            _section(
                '',
                'Nothing says “fiscal discipline” like freezing a junior analyst’s salary while renewing a cloud bill nobody in the boardroom can explain.',
                'Travel gets cut. Hiring gets paused. Employees get a stirring speech about doing more with less.',
                'The database keeps doing more with more.',
                'More scans. More copies. More compute to recalculate yesterday’s answer across five dashboards and a spreadsheet named FINAL_v8_ACTUAL.',
                'Then an executive clicks past the expense chart and lands on a rainforest.',
                'Sustainability.',
                'Beautiful slide. Shame about the operating model.',
            ),
            _section(
                'The waste is coming from inside the leadership team',
                'Bad SQL can start as a technical mistake. Keeping it expensive for three years takes management.',
                'An engineer flags the problem. The fix gets deprioritized. The bill grows. Finance asks questions. Leadership buys more capacity because investigating the waste would threaten the roadmap.',
                'Congratulations. You have put negligence on autopay.',
                'When executives repeatedly refuse to fund a credible fix, that expense belongs to their decision. Stop making the engineering team wear it like a performance issue.',
                'You chose the deadline. You approved the shortcut. You renewed the bill.',
                'Own the damn invoice.',
            ),
            _section(
                'Your cloud provider accepts money for useless work',
                'It will happily process data nobody needed, refresh reports nobody reads, and run six versions of the same calculation because your departments cannot agree on what “customer” means.',
                'The meter has no opinion about your strategy.',
                'That is supposed to be leadership’s job.',
                'Before approving another capacity increase, ask what useful result the existing capacity delivers. If nobody can answer, put the purchase order down long enough to find out.',
                '“Enterprise scale” is an expensive way to pronounce “we stopped checking.”',
            ),
            _section(
                'The rainforest would like a word',
                'The cloud runs on physical infrastructure and electricity. The IEA estimated that data centers consumed about 415 terawatt-hours of electricity in 2024. Your digital transformation has a power supply.',
                'No, we cannot honestly assign a patch of scorched rainforest to your worst query. Environmental impact depends on where and how the work runs, and whether removing it actually reduces resource consumption.',
                'We can, however, identify the absurdity of putting forests in the annual report while treating avoidable computation as someone else’s problem.',
                'If you are worried about rainforests burning, bring that concern into the budget meeting.',
                'Fund the measurement. Fund the cleanup. Give someone responsibility for the result.',
                'A stock photo of a tree is doing a suspicious amount of work in your sustainability strategy.',
                sources=({'title': 'IEA: Energy and AI', 'url': 'https://www.iea.org/reports/energy-and-ai/energy-demand-from-ai'},),
            ),
            _section(
                'Stop buying bigger machines to conceal smaller questions',
                'What does one row represent?',
                'Is this join multiplying records?',
                'Does this report need the entire history?',
                'Who uses it?',
                'How fresh does the answer actually need to be?',
                'These are ordinary questions with financial consequences. They deserve answers before the next infrastructure upgrade.',
                'If a join inflates revenue, a bigger warehouse can help you mislead the board faster. Sprinkling DISTINCT over the wreckage does not constitute financial control.',
                'Pay someone to understand the data.',
                'You can fund that understanding before the report goes out or during the emergency reconciliation afterward. The second option comes with more meetings and substantially worse vibes.',
            ),
            _section(
                'Some of your dashboards need a funeral',
                'Find the report that refreshes every fifteen minutes because someone requested it during a meeting four reorganizations ago.',
                'Find its current owner.',
                'Find the decision it supports.',
                'If neither exists, turn it off.',
                'You do not need a transformation program to stop paying for an abandoned chart.',
                'You need someone with enough authority to say, “This no longer serves a purpose,” and enough organizational support to survive the reply-all.',
                'Retiring useless work should count as delivery. Otherwise, your incentives reward adding machinery forever and punish anyone who reaches for the off switch.',
                'Enjoy financing that.',
            ),
            _section(
                'Make the business case. Then make leadership answer it.',
                'Suppose a validated SQL change removes $7,500 in monthly scan charges. Added storage and refresh costs consume $600. Ongoing engineering upkeep is valued at another $600.',
                'That leaves $6,300 in monthly economic benefit. If implementation costs $6,000, simple payback is roughly one month.',
                'Those are hypothetical figures. In a real proposal, prove the assumptions, preserve output quality, and establish that the charges are avoidable.',
                'Then put the decision in front of the person who keeps saying there is no budget.',
                'They may have a better investment. They may identify a real operational risk. Good. Compare the evidence.',
                'But “we cannot spare the engineering time” deserves scrutiny when the alternative is signing up for the same avoidable expense every month.',
                'You have found money for the problem.',
                'Explain why you cannot fund the solution.',
            ),
            _section(
                'Do not bullshit the savings, either',
                'A faster query under a fixed commitment may release capacity without reducing this month’s bill. Recovered staff time does not automatically become payroll savings. A smaller cloud invoice is not a carbon measurement.',
                'Keep those benefits separate.',
                'And include the upkeep. An optimization that creates a fragile mess of refresh jobs, backfills, and emergency repairs can spend its savings before Finance finishes congratulating you.',
                'Demand evidence from the cleanup proposal. Demand the same evidence from the decision to keep wasting money.',
                'Funny how often only one of those gets interrogated.',
            ),
            _section(
                'Executive accountability starts where the invoice lands',
                'Pick one expensive recurring workload. Name its owner. Establish its purpose. Measure the cost. Fund a justified fix. Verify the result.',
                'There is nothing glamorous about it.',
                'There is also nothing glamorous about laying people off while paying machines to perform work nobody needs.',
                'If you can afford another platform, another capacity increase, and another leadership retreat about operational excellence, you can afford to investigate your own operating expenses.',
                'Your bottom line does not care how visionary the keynote sounded.',
                'The rainforest does not care how green the slide was.',
                'And “we had other priorities” is still a decision.',
                'Put your name on it.',
            ),
        ], offer='diagnostic',
    ),
